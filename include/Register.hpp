...
public:
    ...
    {
        static_assert((std::is_same_v<std::remove_cvref_t<Value>, std::remove_cvref_t<Fields>> || ...), "Bitfield not defined for register");
        return (*rawPtr & Value::mask) >> Value::bitOffset;
    }
    ...
    template<typename T = RegisterType>
        requires details::WriteConcept<T>
